...
11. **Card text shrinks to fit; never pushes the card open.** When a card has a layout-defined size, the text frame uses `<a:normAutofit fontScale="..."/>` so PowerPoint auto-shrinks oversized text within fixed card geometry. Never use `<a:spAutoFit/>`, which resizes the shape to fit the text and breaks the layout grid (cards bleed into neighbors or off the slide). In python-pptx: `text_frame.auto_size = MSO_AUTO_SIZE.TEXT_TO_SHAPE_FIT`. Applies to every text frame inside a fixed-geometry card - titles, bodies, stat numbers, labels.
12. **Stat numbers and headline-scale text (≥ 40 pt) use Signature Gradient text fill in PPTX, not solid blue.** §3 type scale and §6.5 stat-slide spec both call for Signature Gradient on stat numbers; this rule makes it binding for the PPTX path. The text run carries a native PowerPoint gradient fill (`<a:gradFill>` inside `<a:rPr>`, same 30° angle, same `#2665E2 → #C26DE6` stops). Never substitute solid Core Blue "because gradient text is risky" - PowerPoint's native gradient text renders reliably; the §8 safety contract is HTML-only.
